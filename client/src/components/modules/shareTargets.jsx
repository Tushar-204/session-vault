...
export const shareTargets = [
  { key: 'whatsapp', label: 'WhatsApp', color: '#25D366', icon: ICON.whatsapp, build: (url, message) => `https://wa.me/?text=${encodeURIComponent(message)}` },
  { key: 'telegram', label: 'Telegram', color: '#229ED9', icon: ICON.telegram, build: (url, message) => `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(message)}` },
  { key: 'email', label: 'Email', color: '#64748b', icon: ICON.email, build: (url, message, subject) => `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}` },
  { key: 'x', label: 'X', color: '#000000', icon: ICON.x, build: (url, message) => `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(message)}` },
  { key: 'linkedin', label: 'LinkedIn', color: '#0A66C2', icon: ICON.linkedin, build: (url) => `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}` },
  { key: 'facebook', label: 'Facebook', color: '#1877F2', icon: ICON.facebook, build: (url) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}` },
];
